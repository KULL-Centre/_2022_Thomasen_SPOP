...
exp_SAXS = '../../expSAXS_qvalues_1intensities.dat'
drho = 1.0
r0 = 1.025
AA_traj = '../prodrun_AAbackmapped.xtc'
AA_top = '../prodrun_AAbackmapped.gro'
outfile_calc = f'calc_data_{oligomersize}mer.dat'
ouput_avgSAXS = f'SAXS_avg_{oligomersize}mer.dat'
...
for i in range(len(traj)):
    ...
    command = pepsi_path + ' AA_frame.pdb ' + exp_SAXS + ' -o ' + outfile + ' --I0 ' + str(oligomersize) + ' --scaleFactor 1.0 --dro ' + str(drho) + ' --r0_min_factor ' + str(r0) + ' --r0_max_factor ' + str(r0) + ' --r0_N 1'
    os.system(command)
