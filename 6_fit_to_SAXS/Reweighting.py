...
parser = argparse.ArgumentParser(description='This is a program for ensemble refinement of polydisperse oligomers against batch SAXS data. Provided simulated SAXS profiles for each ensemble conformation of each oligomer and an experimental batch SAXS profile, the program returns a set of weights for each oligomer ensemble obtained using BME reweighting. The concentration distribution of oligomers is calculated with the isodesmic oligomerization model, so the total protein concentration and Ka must be provided.')
